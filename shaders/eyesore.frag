    ...
    vec4 sineWave(vec4 pt, vec2 pos)
    {
        if (uampmul > 0.0)
        {
            float offsetX = sin(pt.y * uFrequency + iTime * uSpeed);
            float offsetY = sin(pt.x * (uFrequency * 2.0) - (iTime / 2.0) * uSpeed);
            float offsetZ = sin(pt.z * (uFrequency / 2.0) + (iTime / 3.0) * uSpeed);
